...
Menu::Menu()
{
	...
	// Inicjalizacja zmiennych odpowiedzialnych za poruszanie si� po menu
	buttonId = 0;
	deltaTime = 0.1f;
	heroSpecSet = false;
	menuState = MAIN_MENU;
			...
			case sf::Event::KeyPressed:
			{
				...
				case sf::Keyboard::Return:
				{
					...
					case 4: { std::cout << "Return button has been pressed!" << std::endl; 
						this->setMenuState(MAIN_MENU); moveUp(); moveUp(); moveUp(); return; } // Przy ustawianiu stanu musi by� return;
					default: break;
					}
					break;
				}
...
void NextLevel::draw(sf::RenderWindow & whichWindow)
{
	...
	// Wy�wietlenie dodatkowych efekt�w
	for (std::vector<Enemy*>::iterator it = visualisation.begin(); it != visualisation.end(); ++it)
	{
		(*it)->draw(whichWindow);
	}
